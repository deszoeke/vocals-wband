...
% 2011-02-22 use absolute dBZ for thresholds in v.2.
%
% Average cloud fraction from W-band radar (1-minute resolution)
% to 10 minute resolution. Requires output from proc_wband_cloudfraction_absthr.m
...
T=load([way_proc_data_wband 'cloudfraction/ClFrAbsThresh_1min_2008310-336.txt']);
% same time base as A
cf=T(:,2:end);
thr=-40:2:40; % absolute thresholds
...
% save other threshold cloud fractions in a text file --> version 2
thrfile=[way_proc_data_wband 'cloudfraction/VOCALS2008_WbandClFrAbsThresh10min.txt'];
ff=fopen(thrfile,'w');
fprintf(ff,['%12.8f\t' repmat('%10.5f\t',[1,25]) '%10.5f\n'],...
    [time0_yday_10m, max(-999,cf_10m)]');
fclose(ff);

% save 10-minute cloud fraction in a netcdf file with threshold fractions
...
ncf=[ncpath 'VOCALS2008_WbandCloudFraction10min_2_0.nc']; % v2.0
nc_create_empty( ncf );
nc_add_dimension( ncf, 'time', 0 ) % record dimension
nc_add_dimension( ncf, 'threshold', size(cf_10m,2)) % v1.0a, v2.0
...
nc_attput ( ncf, nc_global, 'version', '2.0a' );
...
nc_addhist( ncf , 'v1.0a: threshold cloud fraction added; v2.0: absolute thresholds=-40:2:40')
