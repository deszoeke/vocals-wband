...
%        total=fft(x(i:(i+nfft-1)).*wind); % try no detrending
        powe=powe+total(2:nfft/2+1).*conj(total(2:nfft/2+1));
    end;
end;
powe=W1*powe/repeats/fs;
% one Nyquist wavenumber coefficient, Nyq-freq. in quadrature with
% discrete sampling not sampled, SPdeS 2013.04.25
% Assumes quadrature-Nyquist frequency power = resolved NF power; or divide last wavenumber by 2
% powe(end)=powe(end)/2;
