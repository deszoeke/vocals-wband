...
%   5 Adjust by -1.72 dB from September calibration by Ken Moran.
%
% Dimensions:
...
%     hh=  sprintf('%02d',floor(mod(base_time_yday,1)*24)); % subject to rounding error!
    ...
    for i=1:length(time_offset_bin)-1
        ...
        if sum(in_bin)>0
            ...
            Z.cfad(:,:,i)=histc(Zrec(in_bin,:),Zbins);
            w.cfad(:,:,i)=histc(wdropmask,wbins);
            Dw.cfad(:,:,i)=histc(wid(in_bin,:),Dwbins);
            
            % statistics over time: mean max min variance skewness
            % 7/12/2010 mod to average Z in linear space rather than dBZ
            Z.mean(i,:)=nanmean(Zrec(in_bin,:));
            Z.min(i,:) =min(Zrec(in_bin,:));
            Z.max(i,:) =max(Zrec(in_bin,:));
            Z.var(i,:) =nanvar(Zrec(in_bin,:),1); % second-moment
            Z.skew(i,:)=skewness(Zrec(in_bin,:));
