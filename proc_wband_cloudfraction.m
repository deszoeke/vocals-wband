...
analog_noise_signal=-120.74; % dBmW peak diagnosed by Simon, dB_offset applied in diagnosis!
...
% other additional thresholds for cloud detection
...
%     fclose(ff);
    ...
    cfile=[way_proc_data_wband 'cloudfraction/' momentfile(fi).name(1:11) 'ClFrThresh.txt'];
    ff=fopen(cfile,'w');
    fprintf(ff,'%7.0f\t%6.4e\t%6.4e\t%6.4e\t%6.4e\t%6.4e\t%6.4e\t%6.4e\n',[time_aggreg,cf]');
    fclose(ff);
end % loop files
...
% end
if ~exist([way_proc_data_wband 'cloudfraction/ClFrThresh2008310-336.txt'],'file')

    filecat([way_proc_data_wband 'cloudfraction/2008*ClFrThresh.txt'],...
            [way_proc_data_wband 'cloudfraction/ClFrThresh_1min_2008310-336.txt'])
end
...
B=load([way_proc_data_wband 'cloudfraction/ClFrThresh_1min_2008310-336.txt']);
