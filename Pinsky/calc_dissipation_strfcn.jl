...
"Search for a file key in path"
searchdir(path,key) = filter(x->contains(x,key), readdir(path))

if isempty(searchdir(".","D_strfcn.mat"))
    ...
    Ddict=matread("D_strfcn.mat")
    # for var in keys(Ddict); print("""$var = Ddict["$var"]\n"""); end
        jmax = Ddict["jmax"]
        ktop = Ddict["ktop"]
        count = Ddict["count"]
        D = Ddict["D"]
        kmax = Ddict["kmax"]
        ydayw10 = Ddict["ydayw10"]
    end
end # big D calculation or read
...
itime=1042; # test time
    ...
    # Negative noise results from a significant number of the series.
    # Each series is grouped to share the same top grid, but should be arranged to share a midpoint.
    # The pickm loop below does this. If we wanted to solve for a global noise coefficient (we probably don't),
    # we'd want to unify columns to share the same midpoint.
