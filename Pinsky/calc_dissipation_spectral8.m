...
% v.6 nwin=192, interpolate only across gaps of >=35 points
% v.7 do not interpolate gaps! Use autocovariance which is insensitive to gaps
% v.8 add w variance and skewness
...
elseif regexp(machine,'farfalla')==1
    stem = '/home/farfalla/data1/sdeszoek/VOCALS/RHB';
    way_Pinsky_retrieval='/home/farfalla/data1/sdeszoek/VOCALS/RHB/Scientific_analysis/programs/VOCALS2008_programs/wband/Pinsky/retrieval/';
end
...
% lok=4;
% hik=16;
% nwin=256; % v.5 double window size to 256 points, 73 s
% lok=4;   % this can stay 4, bc the detrending scales with the window size
% hik=32;  % increase in same proportion
nwin=192; % v4.5  window size to 192 points, 55 s
lok=4;   % this can stay 4, bc the detrending scales with the window size
hik=24;  % increase in same proportion
ninterpgap=20;
...
wvar=NaN+zeros(n10min,kmax);
wskw=NaN+zeros(n10min,kmax);
epsilonk=NaN+zeros(n10min,kmax);
        ...
        if ~isempty(retrfile)
            ...
            for ind=1:round(nt/diten) % index of the 10 minute interval in the hour
                ...
                if length(ii10)>=di/4 % skip to next iteration if window too small
                    ...
                    if ~isempty(ik) && ik<100 % there 10 min cloud data (A cloud top is identified.)
                        ...
                        % v.7 use autocovariance; DO NOT interpolate missing values with gappy_psd
                        isn=isnan(wtop);
                        if sum(~isn)>di/4
                            wtop(isn)=interp1(time_offset(ii10(~isn)),wtop(~isn),time_offset(isn));
                            count(bigind,1)=sum(isfinite(wtop));
                            %S(:)=gappy_psd(wtop,nwin,fs,ninterpgap,detrendit); % 2x faster than pwelch, Hanning window and detrends before FFT, handles gappy data
                            S(:) = fast_acov_spectr2(wtop, nwin, fs); % Ignore Nyquist-frequency power estimate! 
                        ...
                        wvar(bigind,kk) = nanvar(w10);
                        wskw(bigind,kk) = skewness(w10); % ignores NaN by default
                        iwt=isfinite(w10);
                        countk(bigind,kk)=sum(iwt);
                        for k=1:nk % loop through levels
                            if countk(bigind,kk(k))>di/4
                                % v.7 use autocovariance; not gappy_psd interpolates missing values
                    ...
                    end % there is 10 min cloud velocity data
...
save radarturb8.mat ydayw10 F speck epsilonk sigv2k countk knoisek Snoisek Sthreshk wvar wskw
