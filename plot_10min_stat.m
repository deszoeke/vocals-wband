...
A=load('~/Data/cruises/VOCALS_2008/RHB/radar/cband/leg2nocoasts.cz.tilt1.25to60km.stats');
C.year=A(:,1);
C.month=A(:,2);
C.date=A(:,3);
C.hour=A(:,4);
...
n=16;
s=(0:n-1)'/n;
rain=hsv2rgb([0+s/2.5 (.5*(1-cos(pi*s))).^2 .25+.75*s]);
colormap(rain)
...
% compare ceilometer and W-band cloud fraction
