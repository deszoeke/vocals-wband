...
% sync W-band cloud top height cth10 to cloudtop10(time10yday)
...
% sync ceilo cloud base height clb10 to cloudbase10(time10yday)
...
% sync ceilometer cloud fraction ceilfrac10(time10yday)
...
% composite 1-min Z profiles on CMR
cmrbin=[-28 -22 -20 -10 0 10 20]; % 6 bins defined between these 7 endpoints
ntime=length(time_yday);
nheight=length(Z.height);
for indbin=1:length(cmrbin)-1
    ii=zmx>=cmrbin(indbin) & zmx<cmrbin(indbin+1);
    countinbin(indbin)=sum(ii);
    Z.cmrbin(indbin,:)=nanmean(Z.mean(ii,:)); % simple level-mean
    Z.cmrcfad(indbin,:,:)=nansum(Z.cfad(ii,:,:));
...
hold on
plot(Z.cmrbin,Z.height,'linewidth',2)
axis([-50 20 0 2e3])
set(gca,'fontsize',16)
ylabel('altitude (m)')
xlabel('CMR-composite mean reflectivityn (dBZ)')
%print('-dpng',[way_proc_images_wband 'composite_Z_on_CMR.png'])

% CFAD of Z
for indbin=1:6
    ax(indbin)=subplot(2,3,indbin,'align');
    
    [cc,hc]=contourf(Z.bins(2:end-1)+(Z.bins(3)-Z.bins(2))/2,Z.height/1e3,squeeze(Z.cmrcfad(indbin,:,2:end-1)),40);
    set(hc,'edgecolor','none')
    colormap(b2rcolormap(40));
    hb=colorbar('southoutside');
    set(get(hb,'xlabel'),'string','count')
    title(sprintf('CMR=[%d %d]',cmrbin(indbin+[0 1])));
    hold on
    plot(noisefloor,Z.height/1e3,'w-')
    plot(Z.cmrbin(indbin,:),Z.height/1e3,'m-')
    xlabel('dBZ')
    ylabel('height (km)')
end
%print('-dpng',[way_proc_images_wband 'comp_Zcfad_on_CMR.png'])
...
indbin=1;
    axes(ax(2));
    ha=plot(edges(indbin)+.5,-3,'k^','markersize',8,'markerfacecolor','k','clipping','off');
    
    axes(ax(1));
    ii=zmx>=edges(indbin) & zmx<edges(indbin+1);
    ...
    set(gca,'fontsize',16)
    colormap(b2rcolormap(40));
    title(sprintf('CMR=[%d %d]',edges(indbin+[0 1])));
    xlabel('dBZ')
    ylabel('height (km)')
    hb=colorbar('southoutside','fontsize',14);
    set(hb,'position',get(hb,'position')-[0 .08 0 0])
    set(get(hb,'xlabel'),'string','count','fontsize',16)
    hold on
    plot(noisefloor,Z.height/1e3,'w-')
    % make adjustments
    print('-djpeg',[way_proc_images_wband sprintf('CFAD_movie/ZCFAD%02dCMR%+03d.jpeg',indbin,edges(indbin))])
...
movie2avi(frame,[way_proc_images_wband 'CFAD_movie/ZCFAD_on_CMR.avi'],'fps',5);