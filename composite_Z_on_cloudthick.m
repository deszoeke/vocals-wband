% Composite W-band reflectivity Z
read_parameters;
...
% composite 1-min Z profiles on 10-minute cloudthick
thickbin=[100 200 300 400 500 600 700]; % 6 bins defined between these 7 endpoints
ntime=length(time_yday);
nheight=length(Z.height);
for indbin=1:length(thickbin)-1
    ii=cloudthick>=thickbin(indbin) & cloudthick<thickbin(indbin+1);
    ...
    Z.thickbin(indbin,:)=nanmean(Z.mean(jj,:)); % simple level-mean
    Z.thickcfad(indbin,:,:)=nansum(Z.cfad(jj,:,:));
end

h=area([-60; noisefloor],Z.height([1 1:end]),2000,'facecolor',0.7+[0 0 0],'edgecolor','none');
hold on
plot(Z.thickbin,Z.height,'linewidth',2)
axis([-50 20 0 2e3])
set(gca,'fontsize',16)
ylabel('altitude (m)')
xlabel('cloud thickness composite mean reflectivity (dBZ)')
%print('-dpng',[way_proc_images_wband 'composite_Z_on_cloudthick.png'])

% CFAD of Z
for indbin=1:6
    ax(indbin)=subplot(2,3,indbin,'align');
    
    [cc,hc]=contourf(Z.bins(2:end-1)+(Z.bins(3)-Z.bins(2))/2,Z.height/1e3,squeeze(Z.thickcfad(indbin,:,2:end-1)),40);
    set(hc,'edgecolor','none')
    colormap(b2rcolormap(40));
    hb=colorbar('southoutside');
    set(get(hb,'xlabel'),'string','count')
    title(sprintf('z_{top}-z_{base}=[%3.1f %3.1f) km',thickbin(indbin+[0 1])/1e3));
    hold on
    plot(noisefloor,Z.height/1e3,'w-')
    plot(Z.thickbin(indbin,:),Z.height/1e3,'m-')
    xlabel('dBZ')
    ylabel('height (km)')
end
%print('-dpng',[way_proc_images_wband 'comp_Zcfad_on_cloudthick.png'])

% movie of CFADs
edges=0:50:1000;
count=histc(cloudthick,edges);
...
xlabel('cloud thickness (km)') 
set(gca,'tickdir','out')
hold on

indbin=1;
    axes(ax(2));
    ha=plot(edges(indbin)/1e3+.0125,-.3,'k^','markersize',8,'markerfacecolor','k','clipping','off');
    
    axes(ax(1));
    ii=cloudthick>=edges(indbin) & cloudthick<edges(indbin+1);
    ...
    set(gca,'fontsize',16)
    colormap(b2rcolormap(40));
    title(sprintf('z_{top}-z_{base}=[%3d %d) m',edges(indbin+[0 1])));
    xlabel('dBZ')
    ylabel('height (km)')
    hb=colorbar('southoutside','fontsize',14);
    set(hb,'position',get(hb,'position')-[0 .04 0 0])
    ...
    plot(noisefloor,Z.height/1e3,'w-')
    % make adjustments
    print('-djpeg',[way_proc_images_wband sprintf('cloudthick_movie/ZCFAD%02dcloudthick%+03d.jpeg',indbin,edges(indbin))])
...
for indbin=2:length(edges)-1
    ...
    set(gca,'fontsize',16)
    colormap(b2rcolormap(40));
    title(sprintf('z_{top}-z_{base}=[%d %d)',edges(indbin+[0 1])));
%     xlabel('dBZ')
%     ylabel('height (km)')
    hb=colorbar('southoutside','fontsize',14);
    set(hb,'position',get(hb,'position')-[0 .04 0 0])
...
movie2avi(frame,[way_proc_images_wband 'cloudthick_movie/ZCFAD_on_cloudthick.avi'],'fps',5);