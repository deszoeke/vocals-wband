...
% radar data files
% it=18192;
% wanttime=Z.time_yday(it);
% ind=768;
% % momentfile(ind).name;
momentfile=[way_raw_data_wband '20083270400MMCRMom.nc' ];
...
drizzle= [ 0.3137    0.3176    0.3137
           ...
           1.0000    0.8000         0
           1.0000    0.6000         0 ];
colormap([gray(16); b2rcolormap(17)]);
...
%% plot timeheight
subplot(3,1,1)
imagesc(time_offset/60,h(1:73)/1e3,Z(:,1:73)'); axis xy;
colormap([gray(12); b2rcolormap(13)]);
caxis([-34 14])
hc=colorbar; set(hc,'tickdir','out','fontsize',14)
set(get(hc,'ylabel'),'string','reflectivity (dBZ)')
set(gca,'xlim',[43 43+6],'ylim',[0 1.8],'fontsize',14,'color','k','tickdir','out');
% title('reflectivity (dBZ)')
% xlabel('22 Nov 2011 04:43-04:49 UTC')
% ylabel('height (km)')
orient tall
print('-depsc',[put_gordon '2011nov220446_1.eps'])

subplot(3,1,2)
imagesc(time_offset/60,h(1:73)/1e3,-w(:,1:73)'); axis xy;
b=b2rcolormap(23);
colormap(brighten([0 0 0; b([1:10 13:22],:)],-.1)); caxis([-5.5 5])
hc=colorbar; set(hc,'tickdir','out','fontsize',14)
set(get(hc,'ylabel'),'string','Doppler velocity (m s^{-1})')
set(gca,'xlim',[43 43+6],'ylim',[0 1.8],'fontsize',14,'color','k','tickdir','out');
% title('Doppler velocity (m s^{-1})')
% xlabel('22 Nov 2011 04:43-04:49 UTC')
ylabel('height (km)')
print('-depsc',[put_gordon '2011nov220446_2.eps'])

subplot(3,1,3)
imagesc(time_offset/60,h(1:73)/1e3,width(:,1:73)'); axis xy;
colormap([0 0 0; b2rcolormap(27); .5 .5 .5]); caxis([-0.1 2.7])
hc=colorbar; set(hc,'tickdir','out','fontsize',14)
set(get(hc,'ylabel'),'string','Doppler width (m s^{-1})')
set(gca,'xlim',[43 43+6],'ylim',[0 1.8],'fontsize',14,'color','k','tickdir','out');
% title('Doppler width (m s^{-1})')
xlabel('22 Nov 2011 04:43-04:49 UTC')
% ylabel('height (km)')
print('-depsc',[put_gordon '2011nov220446_3.eps'])

%% old style for 4-hour section
clf 
...
axis([4 8 0.1 2])
caxis([-50 30])
hb(1)=colorbar; set(hb(1),'ylim',[-50 -10])
...
axis([4 8 0.1 2])
caxis([-9 3])
hb(2)=colorbar; set(hb(2),'ylim',[-3 3])
set(gca,'color','k','fontsize',14,'tickdir','out')
title('vertical velocity (m/s)')
xlabel('Nov 18 hour')
ylabel('height (km)')
set(gcf,'inverthardcopy','off','color','w')
% print('-dpng',[way_proc_images_wband 'wband_nov18.png'])
% delete(hpc(:))
% print('-depsc',[way_proc_images_wband 'wband_nov18.eps'])

%% plot Doppler velocity and width 
colormap([[0 0 0]; b2rcolormap(17)]);
ti=rem(time_mld,1)*24>=6.6 & rem(time_mld,1)*24<=6.9;
ax(1)=subplot(2,1,1,'align');
hpc(1)=imagesc((rem(time_mld(ti),1)*24-6)*60,h/1e3,-w(ti,:)'); shading flat
axis xy
axis([36 54 0.1 2])
caxis([-2.25 2])
hb(2)=colorbar; set(hb(2),'ylim',[-2.25 2],'fontsize',14)
set(gca,'color','k','fontsize',14,'tickdir','out')
title('vertical velocity (m/s)')
% xlabel('Nov 20 hour')
ylabel('height (km)')
...
ax(2)=subplot(2,1,2,'align');
hpc(2)=imagesc((rem(time_mld(ti),1)*24-6)*60,h/1e3,width(ti,:)');
axis xy
axis([36 54 0.1 2])
caxis([-.0625 1])
hb(2)=colorbar; set(hb(2),'ylim',[-0.0625 1],'fontsize',14)
set(gca,'color','k','fontsize',14,'tickdir','out')
title('Doppler width (m/s)')
xlabel('Nov 20 11 UTC minute')
ylabel('height (km)')
set(gcf,'inverthardcopy','off','color','w')
% print('-depsc',[way_proc_images_wband 'vel_width_nov18.eps'])

%%%%% END time height series plots %%%%%

%% velocity vs. Z joint histograms -slow
...
orient tall
% print('-depsc',[way_proc_images_wband 'joint_w_Z_nov18.eps'])
% print('-dpng',[way_proc_images_wband 'joint_w_Z_nov18.png'])
...
plot(noisefloor,h,'y-')
set(gca,'ylim',[0 2000])
title({'2008 Nov 18' 'reflectivity log-CFAD'})
xlabel('dBZ')
subplot(3,2,3,'align')
